...
Implement guided weekly review workflow with 4 steps.

## Input

Task L7-40, L7-38, L7-39 completed.

## Output

`src/app/(dashboard)/review/page.tsx` with review page.
...
export default function ReviewPage() {
  ...
  const handleSetNextAction = (task: any) => {
    updateTask.mutate({ id: task.id, data: { is_next_action: true } });
  };
    ...
    <div className="space-y-6">
      ...
      {step === 'next-actions' && (
        <section>
          ...
          <p className="mb-4 text-foreground-secondary">
            Select tasks for the next week by marking them as Next Actions.
          </p>
          <TaskList
            tasks={inboxTasks || []}
